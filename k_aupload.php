...
if(isset($_POST['upload'])){
    ...
    if(in_array($imgfiletype,$extensions_arr)){
        if(move_uploaded_file($_FILES['ImageFile']['tmp_name'],$img_dir.$imgname)){
            $sql = "insert into kitchen_aupload(imgname, productprice,productname, itemleft) values('$imgname', '$productprice', '$productname', '$itemleft')";
            if (mysqli_query($conn,$sql)) {
                // echo "<script>alert('Image Uploaded Successfully')</script>";
                header("Location:admink_aupload.php");
            }else{
